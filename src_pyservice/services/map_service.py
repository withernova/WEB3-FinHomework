...
class MapHeatAIAssessor:
    ...
    def batch_score(self, marker_list: list) -> list:
        ...
        # 构建批量 prompt
        prompt = (
            "你是一名专业地图分析AI。我们正在执行一项搜救任务，搜救员会将搜索过的区域在地图上进行标记。每个标记有类型和描述，请根据规则为每个标记评分：\n"
            "【标记类型说明】\n"
            "- searched：已搜索区域，通常代表该区域已经被搜查过，优先级低，通常不再需要更多人力。\n"
            "- danger：危险区域，代表这里有危险或需重点关注，通常需要较多的人力资源。\n"
            "- poi：兴趣点，通常是需要关注、可能有被困人员的区域，优先级较高。\n"
            "- path：搜索路径，表示已规划或已执行的搜索线路，通常优先级较低。\n"
            "【评分规则】\n"
            "- 分值区间为[-1.0, 1.0]。\n"
            "- 已搜索区域、已覆盖区域、描述中包含“已搜索”“已排查”等字样，评分应靠近-1.0（负值）。\n"
            "- 危险区域、需要重点关注或人力的区域，评分应靠近1.0（正值）。\n"
